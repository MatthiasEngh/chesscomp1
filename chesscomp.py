...
def move_minimum(move1, board):
  ...
  next_turn_options_count = [[move2, count_responses(move2, board)] for move2 in board.legal_moves]
  result = next_turn_options_count[0][1]
  for evaluation in next_turn_options_count[1:]:
    if result < evaluation[1]:
      pass
    elif result == evaluation[1]:
      result = random.choice([result, evaluation[1]])
    else:
      result = evaluation[1]
