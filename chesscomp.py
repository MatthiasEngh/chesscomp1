...
def mate_or_minimize(board):
  try:
    result = minimize(legal_moves(board), count_moves_eval)[1]
  except StopIteration:
    result = 9001
  return result

def minimized_personal_responses(board):
  for move in board.legal_moves:
    board.push(move)
    result = mate_or_minimize(board)
    yield [move, result]
    board.pop()
...
def find_move_with_max_next_turn_options(board):
  moves_with_minimized_responses = minimized_personal_responses(board)
